    ...
    <>
      ...
      <AdminSection description={`${courses.pagination.totalCount} مقررًا مطابقًا للمرشحات الحالية.`} title="المقررات">
        {courses.data.length === 0 ? <AdminEmptyState body="لا توجد مقررات ضمن هذه الحالة أو معايير البحث." title="لا توجد مقررات مطابقة" /> : (
          <><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>المقرر</th><th>المدرّس</th><th>الحالة</th><th>جاهزية النشر</th><th>آخر نشر</th><th>الإجراءات</th></tr></thead><tbody>{courses.data.map((course) => <tr key={course.id}><td>{course.title}<small><bdi dir="ltr">{course.courseCode ?? course.slug}</bdi></small></td><td>{course.instructorName}</td><td><AdminStatus>{courseStatusLabel[course.status]}</AdminStatus></td><td><AdminStatus>{course.readiness.canPublish ? "جاهز" : "يحتاج إكمال"}</AdminStatus></td><td>{formatAdminDate(course.publishedAt)}</td><td><AdminCourseActions canPublish={course.readiness.canPublish} courseId={course.id} courseTitle={course.title} currentStatus={course.status} /></td></tr>)}</tbody></table></div><AdminPagination nextHref={courses.pagination.page < courses.pagination.totalPages ? hrefForPage(courses.pagination.page + 1) : null} page={courses.pagination.page} previousHref={courses.pagination.page > 1 ? hrefForPage(courses.pagination.page - 1) : null} totalPages={courses.pagination.totalPages} /></>
        )}
      </AdminSection>
    </>
  );
