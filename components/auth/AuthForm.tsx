...
export function AuthPanel({ mode, redirectTo = "/account" }: { mode: CurrentAuthMode; redirectTo?: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isPasswordUpdate && password !== confirmPassword) { setFieldErrors({ confirmPassword: "كلمتا المرور غير متطابقتين." }); return; }
    setIsSubmitting(true);
    const body = { ...(!isPasswordUpdate ? { email: String(formData.get("email") ?? "") } : {}), ...(hasPassword ? { password } : {}), ...(isRegister ? { fullName: String(formData.get("fullName") ?? "") } : {}) };
    try {
      ...
      if (isRegister) { setPendingConfirmationEmail(String(formData.get("email") ?? "")); setMessage("تحقق من بريدك الإلكتروني لتفعيل الحساب، ثم سجّل دخولك."); }
  ...
  async function handleResend() {
    ...
    try {
      const { response, payload } = await post("/api/auth/resend-confirmation", "POST", { email: pendingConfirmationEmail });
  ...
  return <section className={styles.card} aria-labelledby="auth-heading">
    <div className={styles.heading}><h1 id="auth-heading">{content.title}</h1><p>{content.description}</p></div>
    ...
    <nav className={styles.links} aria-label="روابط الحساب">{mode === "sign-in" ? <><Link href="/auth/register">حساب جديد</Link><Link href="/auth/password-reset">نسيت كلمة المرور؟</Link></> : <Link href="/auth/sign-in">العودة إلى تسجيل الدخول</Link>}</nav>
  </section>;
}
